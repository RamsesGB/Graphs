...
#Executes as long as graph is smaller than 500
while len(graph) < len(roomGraph):

    currentRoom = player.currentRoom.id

    #verifies the current room the player is in is inside the 'graph' dictionary.
    ...
    for path in graph[currentRoom]:
        ...
        if path not in graph[currentRoom]:
            break
        #Moves player in current direction as long as there is a room to travel to
        if graph[currentRoom][path] == "?":
            ...
            #Take new room and reset it to current room
